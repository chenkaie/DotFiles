...
UseVimball
finish
syntax/hicolors.vim	[[[1
136
" hicolors.vim:
"   Author:	Charles E. Campbell, Jr.
"   Date:	Sep 01, 2009
"  Usage:  :he[lp] hicolors
" =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
" GetLatestVimScripts: 1081 1 :AutoInstall: hicolors.vim
"
" History:
"  v6: Dec 01, 2008 saves and restores user's updatetime setting
...
"  call Dfunc("HighlightColors() line=".curline." col=".curcol)
  if !exists("s:lzkeep")   |let s:lzkeep    = &lz   |endif
  if !exists("s:mousekeep")|let s:mousekeep = &mouse|endif
...
" ---------------------------------------------------------------------
" s:HiColorsRestore: {{{1
fun! s:HiColorsRestore()
"  call Dfunc("s:HiColorsRestore()")
  if exists("s:updatetime")
   let &updatetime= s:updatetime
   unlet s:updatetime
  endif
  if exists("s:lzkeep")   |let &lz   = s:lzkeep    |unlet s:lzkeep   |endif
  if exists("s:mousekeep")|let &mouse= s:mousekeep |unlet s:mousekeep|endif
"  call Dret("s:HiColorsRestore")
endfun
...
augroup HICOLORS
 au!
 au BufUnload,BufLeave	hicolors.txt	call <sid>HiColorsRestore()
 au CursorHold,BufEnter	hicolors.txt	set lz|call <SID>HighlightColors()|set nolz
augroup END
...
vim: ft=hicolors
*hicolors*
AltAltUnique  DiffChange  lCursor     Question        StatusLineNC  User2
AltConstant   DiffDelete  LineNr      Red             StorageClass  User3
AltFunction   DiffText    Magenta     Repeat          String        User4
AltType       Directory   MatchParen  Scrollbar       Structure     User5
AltUnique     Error       Menu        Search          Subtitle      User6
Blue          ErrorMsg    ModeMsg     SignColumn      TabLine       User7
Boolean       Exception   MoreMsg     Special         TabLineFill   User8
Comment       Float       NonText     SpecialChar     TabLineSel    User9
Constant      FoldColumn  Normal      SpecialComment  Tag           VertSplit
Cursor        Folded      Operator    SpecialKey      Title         Visual
CursorColumn  Function    Pmenu       SpellBad        Todo          VisualNOS
CursorLine    Green       PmenuSbar   SpellCap        Type          WarningMsg
Cyan          Identifier  PmenuSel    SpellLocal      Typedef       White
Debug         Ignore      PmenuThumb  SpellRare       Underlined    WildMenu
Delimiter     IncSearch   PreCondit   Statement       Unique        Yellow
DiffAdd       Label       PreProc     StatusLine      User1

    leftmouse,?    : help for given colorname if any is available
                     use ctrl-o to return
    rightmouse,<cr>: brings up the color editor
    see "hicolorshelp" for help with hicolors itself
ftplugin/hicolors.vim	[[[1
1106
" hicolors.vim : colorscheme editor
"   ***		This file is intended to go into .vim/ftplugin/
"  Author:	Charles E. Campbell, Jr.
"  Date:	Sep 02, 2009
"  Version:	9k	ASTRO-ONLY
"
...
"  call Dfunc("WriteColorscheme()")
  ...
  silent! %s/ xxx / /
  silent! %s/^/hi /
  silent! g/\<hiBar\(Red\|Green\|Blue\)\d*\>/d
  silent! g/hiLocalColor/d
  silent! g/\<hiBar\d\+/d
  silent! g/\<cleared\>/d
  silent! g/font=/s/\s*font=.*$//

  " get new colorscheme name
  let newschemename= substitute(input("Enter new colorscheme name: "),'\.vim$','','e')
  if filereadable(newschemename.".vim")
   let overw= input("Do you want to overwrite <".newschemename.".vim>? [y|n] ")
   if overw !~ '^[yY]'
...
"   call Decho("newschemename<".newschemename.">")
   if expand("$USER") != ""
   ...
   silent! %s/\^\[//g
"   call Decho("exe silent wq ".newschemename.".vim")
