...
function! s:OpenSession(name)
	...
	if a:name != '' && a:name[0] != '"'
		...
		let n = bufnr('%')
		"Kent, replace 'silent! so ' with 'so ' to avoid open the same files simultaneously.
		execute 'so ' . s:sessions_path . '/' . a:name
...
"Addy by Kent for shortly mapping.
command! -nargs=1 -complete=custom,s:SessionOpenComplete SOpen call s:OpenSession(<f-args>)
command! -nargs=0 SOpenLast if exists('g:LAST_SESSION') | call s:OpenSession(g:LAST_SESSION) | endif
command! -nargs=0 SClose call s:CloseSession()
command! -nargs=0 SList call s:ListSessions()
command! -nargs=0 SSave call s:SaveSession()
command! -nargs=? SSaveAs call s:SaveSessionAs(<f-args>)
command! -nargs=0 SShowLast call s:ShowLastSession()
